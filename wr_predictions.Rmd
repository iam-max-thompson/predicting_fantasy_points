...
wr_by_season <- wrs %>% group_by(player_id, season) %>% summarize(total_pts = sum(fantasy_points_ppr), total_games = n(), points_per_game = total_pts / total_games, total_targets = sum(targets), targets_per_game = total_targets / total_games, wopr_per_game = sum(wopr) / total_games, epa_per_game = sum(receiving_epa) / total_games, air_yards_per_game = sum(receiving_air_yards) / total_games, total_wopr = sum(wopr))


# everything before 2021 only had 16 games
...
# currently missing 11 rows post 2012???

# JoneDo00 - donald jones
# AdamKy00 -- kyle adams
#WillDe00
#WillRo00
#DaviRa00
...
nas <- post2012 %>% filter(is.na(season_snaps))# %>% #select(display_name, new_pfr_id, player_id, season) %>%
...
wr_attempt_table$is_on_new_team <- ifelse(wr_attempt_table$pass_attempt_difference == 0, 0, 1)
...
```{r}
#######TODO: try to get targets ADDED from last year 
...
#making the pre-2021 adjustment for wr_by_yr
#wr_by_yr$targets <- ifelse(wr_by_yr$season < 2021, wr_by_yr$targets * 17/16, wr_by_yr$targets)
...
targets_last_yr <- wr_by_yr %>% group_by(final_team, season) %>% summarize(total_targets_last_year = sum(targets))

targets_last_yr$season <- targets_last_yr$season + 1

target_dropoff <- tm_targets_remaining %>% left_join(targets_last_yr, by = c("season", c("starting_team" = "final_team"))) %>% select(starting_team, season, remaining_targets_last_year, total_targets_last_year, targets_added_this_year) %>% mutate(target_dropoff =  total_targets_last_year - remaining_targets_last_year)
...
espn_qbr <- espn_qbr %>% arrange(player_id, season) %>%
  group_by(player_id) %>% mutate(starter_qbr_ly = lag(qbr_total, n=1)) %>% ungroup()
...
espn_qbr <- espn_qbr %>% select(season, team_abb, starter_qbr_ly, qbr_ly_bin)
...
model <- train(points_per_game ~ points_ly + targets_pg_ly + wopr_pg_ly + pick + air_yards_pg_ly + total_games_ly  + pass_attempt_difference + position + total_positional_investment + target_dropoff + points_2y + total_games_2y + points_3y + total_games_3y + targets_pg_2y + targets_pg_3y + epa_pg_ly + years_pro + wopr_pg_2y + wopr_pg_3y + targets_added_this_year + is_on_new_team + combine_cluster + points_per_snap_ly + targets_per_snap_ly + wopr_per_snap_ly + total_snaps_ly  + qbr_ly_bin, data = trainData, 
...
prof <- model_profile(dalex_model, type = "accumulated", variable = "qbr_ly_bin")
plot(prof)
...
a <- testData %>% arrange(desc(abs(residual))) %>% select(starting_team, pfr_player_name, season, points_per_game, predicted, residual, total_games, points_ly, player_id) %>% head(20)
