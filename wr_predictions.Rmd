...
wr_by_yr <- wrs %>%
  ...
  arrange(player_id, season) %>%  # Sort the data for correct lagging
...
wr_by_yr$targets_same_team_last_year <- ifelse(is.na(wr_by_yr$targets_same_team_last_year), 0, wr_by_yr$targets_same_team_last_year)

tm_targets_remaining <- wr_by_yr %>% group_by(starting_team, season) %>% summarize(remaining_targets_last_year = sum(targets_same_team_last_year), targets_added_this_year = sum(targets_for_new_team, na.rm = T),
...
tm_targets_remaining <- tm_targets_remaining %>% mutate(remaining_fp_last_year = ifelse(starting_team == "GB" & season == 2016, 940, remaining_fp_last_year))

tm_targets_remaining <- tm_targets_remaining %>% mutate(remaining_targets_last_year = ifelse(starting_team == "GB" & season == 2016, 510, remaining_targets_last_year))
...
ggplot(rookies, aes(x=factor(round), y = points_per_game)) + geom_boxplot()
...
cluster_breakdown <- complete_combine_wr %>% group_by(cluster) %>% summarize(avg_weight = round(mean(wt),0), avg_height = round(mean(ht_in),1), avg_forty = round(mean(forty),2), avg_vertical = round(mean(vertical),1), avg_shuttle = round(mean(shuttle),2))

#write.csv(cluster_breakdown, "cluster_breakdown.csv")
...
```{r}
# scatterplot of points per game ly vs this year, no rookies
to_plot <- MASTER %>% filter(years_pro > 0)

ggplot(to_plot, aes(x = points_pg_ly, y = points_per_game)) + 
  geom_point(size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE, lwd = 2) +
  labs(x = "Points per Game Last Year", y = "Points Per Game") +
  theme(axis.title = element_text(size = 14))  # Adjust the size (you can change '14' to your desired size)

M <- lm(points_per_game ~ points_pg_ly, data = to_plot)
summary(M)
...
```{r}
# columns to predict:
#points_per_game, points_pg_ly , targets_pg_ly , wopr_pg_ly , pick , air_yards_pg_ly , total_games_ly  , pass_attempt_difference  , total_positional_investment , target_dropoff , epa_pg_ly , years_pro , targets_added_this_year , is_on_new_team , points_per_snap_ly , targets_per_snap_ly , wopr_per_snap_ly , total_snaps_ly  , total_passing_tds_ly , total_passing_yds_ly , total_passing_fp_ly , is_returning_coach , hc_years_with_team , catch_rate_ly , position , fp_dropoff , starter_epa_passing_ly , starter_epa_persnap_passing_ly , combine_cluster , qbr_ly_bin,  points_2y , total_games_2y , points_3y , total_games_3y , targets_pg_2y , targets_pg_3y , wopr_pg_2y , wopr_pg_3y, season, pfr_player_name, starting_team
```
